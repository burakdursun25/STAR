...
class SkeletonConfig:
    ...
    BONE_PAIRS = [
        ...
        # Baş
        (PoseLandmarks.NOSE.value, PoseLandmarks.LEFT_EAR.value),
        (PoseLandmarks.NOSE.value, PoseLandmarks.RIGHT_EAR.value),
    ]
    
    # Blender uyumlu kemik isimleri
    BONE_NAMES = {
        (11, 12): "Chest",
...
class SkeletonProcessor:
    ...
    @staticmethod
    def calculate_angle(p1: Vector3, center: Vector3, p2: Vector3) -> float:
        """Üç nokta arasındaki açı hesapla (radians)"""
        v1 = p1 - center
        v2 = p2 - center
        ...
        dot = v1_norm.x * v2_norm.x + v1_norm.y * v2_norm.y + v1_norm.z * v2_norm.z
        dot = max(-1, min(1, dot))  # Clamp [-1, 1]
    ...
    def process_landmarks(self, landmarks: List[LandmarkPoint]) -> SkeletonFrame:
        ...
        for start_idx, end_idx in SkeletonConfig.BONE_PAIRS:
            ...
            bone_name = SkeletonConfig.BONE_NAMES.get(pair_key, f"Bone_{start_idx}_{end_idx}")
...
# ============================================================================
# COMMUNICATION - BLENDER'E VERI GÖNDER
# ============================================================================

class BlenderPublisher:
    """Blender'e UDP üzerinden iskelet veri gönder"""
    ...
    def __init__(self, host: str = BLENDER_HOST, port: int = BLENDER_PORT):
        ...
        print(f"[BlenderPublisher] Blender'e bağlanması bekleniyor: {host}:{port}")
    
    def send_skeleton(self, skeleton_frame: SkeletonFrame) -> None:
        """İskelet frame'ini Blender'e gönder"""
        try:
            data = skeleton_frame.to_dict()
            payload = json.dumps(data).encode("utf-8")
            self.socket.sendto(payload, self.target)
        except Exception as e:
            print(f"[BlenderPublisher Error] {e}")
...
class BlenderLiveRunner:
    """Blender'e canlı iskelet veri gönderen runner"""
    ...
    def __init__(self, camera_index: int = CAM_INDEX):
        ...
        self.publisher = BlenderPublisher()
        ...
        print(f"[BlenderLiveRunner] Başlatıldı - Kamera {camera_index}, FPS {self.target_fps}")
    
    def run(self) -> None:
        """Ana loop - kamera akışını işle ve Blender'e gönder"""
        frame_count = 0
        last_time = time.time()
        
        print("[BlenderLiveRunner] Başlıyor... Q tuşuna basarak çık")
        ...
        try:
            while True:
                ...
                # Blender'e gönder
                self.publisher.send_skeleton(skeleton_frame)
                
                # Görüntüsü göster
                display_frame = self.visualizer.draw_skeleton(frame, landmarks)
                
                # İstatistikleri ekrana yazı olarak ekle
                cv2.putText(display_frame, f"FPS: {skeleton_frame.frame_number}", 
                ...
                cv2.imshow("Skeleton -> Blender Live", display_frame)
    ...
    def cleanup(self) -> None:
        """Kaynakları temizle"""
        print("[BlenderLiveRunner] Kapatılıyor...")
        self.capture.release()
        self.processor.close()
        self.publisher.close()
        cv2.destroyAllWindows()
        print("[BlenderLiveRunner] Kapatıldı.")
...
if __name__ == "__main__":
    runner = BlenderLiveRunner(camera_index=CAM_INDEX)
    runner.run()
